        ...
        def ret(*args, **kwargs):
            for _ in range(3):
                try:
                    ...
                    g_logger.debug(f"> GitHub Search and/or Core Rate limit exceeded")
                    g_logger.debug(f"> Reset is in {seconds:.3g} seconds.")
                    if seconds < 0:
                        seconds = 1

                    if seconds > 0.0:
                        g_logger.debug(f"> Waiting for {seconds:.3g} seconds...")
                        time.sleep(seconds)
                        g_logger.debug("> Done waiting - resume!")
